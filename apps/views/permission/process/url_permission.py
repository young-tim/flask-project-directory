...
def get_url():
    ...
    # url = SysUrlsModel.query.filter_by(id=id).first()
    # 两者写法结果相同
    url = SysUrlsModel.query.get(id)
    if url:
        # url["_id"] = str(url["_id"])
        ...
        data = {"msg":"No relevant data found", "msg_type":"w", "code":404}
    return data
...
def get_urls():
    ...
    data["urls"] = datas_paging(pre=pre, current=page, total=data_cnt, datas=urls)
    return data
...
def add_url():
    ...
    s, r = arg_verify([("routing", url)], required=True)
    if not s:
        return r
    url = url.strip()
    if mdb_sys.db.sys_urls.find_one({"url": url}):
        data = {'msg': "Routing already exists", 'msg_type': "w", "code": 403}
    else:
        ...
        if r.inserted_id:
            data = {"msg": "Added successfully", "msg_type": "s", "code": 201,
                    "inserted_id":str(r.inserted_id)}
        else:
            data = {"msg": "Add failed", "msg_type": "w", "code": 400}
    return data
...
def update_url():
    ...
    s, r = arg_verify([("method", method)], required=True,
    ...
    if r.modified_count:
        ...
        data = {"msg":"Modify the success", "msg_type":"s", "code":201}
    else:
        data = {"msg": "No modification", "msg_type": "w", "code": 400}
    return data
...
def delete_url():
    ...
    if r.deleted_count:
        ...
        data = {"msg":"Successfully deleted", "msg_type":"s", "code":204}
    else:
        data = {"msg": "Delete failed", "msg_type": "w", "code": 400}
    return data