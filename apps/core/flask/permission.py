...
from apps.configs.config import GET_DEFAULT_SYS_PER_CACHE_KEY, GET_ALL_PERS_CACHE_KEY
from apps.core.flask.response import response_format
from apps.configs.site_config import SITE_CONFIG
from apps.models.sys import SysUrlsModel
from apps.models.user import UserPermissionModel
    ...
    def decorator(f):
        ...
        def decorated_function(*args, **kwargs):
            ...
            if custom_per:
                ...
                r = True
            if not r:
                return response_format({"msg":'Permission denied,requires "{}" permission'.format(keys),
                        "msg_type":"w", "code":401})
    ...
    def decorator(f):
        ...
        def decorated_function(*args, **kwargs):
            ...
            if custom_login_required and current_user.is_anonymous:
                return redirect(SITE_CONFIG["login_manager"]["LOGIN_VIEW"])
            custom_per = custom_url_permissions()
            if custom_per:
                r = current_user.can(custom_per)
                if not r:
...
def get_permissions_default():
    ...
    :return:
    '''
    pers = UserPermissionModel.query.filter(UserPermissionModel.value.isnot(None), UserPermissionModel.is_default == 1).all()
    value = 0b0
    for per in pers:
...
def get_permissions():
    ...
    :return:{name:<value>}
    '''
    pers = UserPermissionModel.query.filter(UserPermissionModel.value.isnot(None)).all()
    value = {}
    for per in pers:
...
def get_sys_url(url):
    ...
    :return:
    '''
    value = SysUrlsModel.query.filter_by(url=url).first()
