...
class Cache():
    ...
    def __init__(self, app=None):
        # 如果不想让结果为None，则可以使用cache空类
        # self.cache_none = type(CacheNone())
        self.cache_none = None
        if app is not None:
            self.init_app(app)
    ...
    def _get_interface(self, app):
        ...
        # self.config.setdefault('CACHE_MONGODB_DB', "")
        # self.config.setdefault('CACHE_MONGODB_COLLECT', "")
        # 所有键(key)之前添加的前缀,这使得它可以为不同的应用程序使用相同的缓存服务器
        self.config.setdefault('CACHE_KEY_PREFIX', 'x-cache:')
    ...
    def cached(self, timeout=None, key=None, key_base64=True, db_type=None, key_prefix="", is_class_func=False):
        ...
        :param key: 默认使用func的参数拼接作为key, 没有参数则使用为'web/{}'.format(request.path)
